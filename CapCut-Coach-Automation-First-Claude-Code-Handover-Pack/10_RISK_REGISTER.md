...
|---|---|---:|---|---|
| Cold-start edit is weak | Medium | High | Format playbooks; three candidates; uncertainty choices; factual/script constraints; golden fixtures | User selects/revises; retain deterministic Clean cut; learn from outcome |
| Local final renderer differs from CapCut look | Medium | Medium | RenderGraph golden tests; ASS/SVG design templates; calibrated colour/audio; visual regression | Provide optional CapCut Free touch-up package |
| Coding agent adds a paid dependency | Medium | Critical | Zero-cost ADR, lockfile/licence audit, prohibited-service tests, no payment credentials | Block release and replace with local/free component |
...
| Optional CapCut resource ids change or expire | Medium | Medium | Never depend on them; exact-version canary for optional templates | Omit affected element and keep local-render workflow |
...
- manual CapCut Free steps where Accessibility mapping is unstable;
