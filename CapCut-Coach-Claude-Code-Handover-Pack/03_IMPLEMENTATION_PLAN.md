...
## Phase 0 — Evidence and compatibility spike

### Objective

Determine what is safely possible on the owner's exact M2 Mac and installed CapCut build before committing to direct draft automation.
...
- Detect CapCut bundle path, bundle version and build number from `Info.plist`.
- Detect whether CapCut is International (`app_source == cc`) rather than JianYing.
- Locate project roots through known paths and user selection; do not assume one hardcoded path.
...
#### P0.2 Safe upstream trial

- Install/pin `capcut-cli` in a disposable tool workspace.
- Create a new throwaway five-second CapCut project manually containing one video and one caption.
- Close CapCut.
- Duplicate the complete project directory.
- Run read-only `info`, `version`, `diagnose` and `lint` operations.
- Save redacted results to `docs/capcut-compatibility.md`.
- Identify the canonical timeline file(s), mirror files and app version markers.

#### P0.3 Mutation canary

- On the duplicate only, change one caption using the safest supported command.
- Refuse if the tool reports write-guarded/unsupported.
- Open CapCut, inspect, save, close and reopen.
- Verify media, timing and caption.
- Restore the backup and verify hashes.
- Automate the procedure and repeat it ten times.

#### P0.4 Accessibility probe

- Build a minimal Swift CLI that lists the frontmost CapCut window and accessible controls after permission is granted.
...
- `scripts/doctor.sh`
- `docs/capcut-compatibility.md`
- `docs/m2-benchmark.md`
- CapCut fixture bundle with media excluded/redacted
- `apps/mac-bridge` read-only probe
- documented decision: direct writes enabled or disabled for this exact version

### Exit gate

No direct-write code enters the normal workflow until the ten-run canary passes. Read-only and safe handoff may proceed regardless.
...
Deliver value before preference learning or direct CapCut writes.
...
## Phase 5 — Paired raw/final alignment and universal Style DNA

### Objective

Learn the editor's fingerprint from 15–20 examples without opaque model fine-tuning.
...
Make the transition into CapCut as automatic as safely possible.
...
### P7C Direct template clone — only if Phase 0 passed

- Create immutable Premium master-template registry.
...
## Phase 8 — Guided CapCut Premium Mode
...
- The user can install, create an edit, continue in CapCut, export, review and recover without Terminal.
...
2. Use CapCut Coach in CapCut Finish mode.
...
- Several focused days: environment probe plus basic rough-cut proof.
- Approximately 1–2 weeks: useful local first version with transcription, rough cut, SRT, previews and safe handoff.
- Approximately 4–8 weeks: robust Style DNA, direct adapter where compatible, Guided Mode, QC and packaging.
