...
5. Execute Phase 0 before building direct CapCut writes.
...
5. Safe handoff as permanent fallback.
...
> Read the handover pack and repository. Execute Phase 0 only. Build a read-only Mac/CapCut doctor, create a disposable-project diagnostic procedure, record exact app/schema evidence, and implement a caption-only canary that refuses unsupported versions and never targets an original. Do not force writes. Do not proceed to normal direct integration until the ten-run canary and restore test pass. Continue with environment-independent fixture/tests while waiting for user actions.
...
> Implement Phase 5 using paired raw/final alignment. Build audio-first matching with visual verification, per-example reports, robust feature aggregation, quality weights, approved Style DNA versions and A/B feedback. Do not fine-tune a foundation model. Low-confidence inferences remain suggestions.
