...
No release is complete until these tests are run and results are recorded. Tests requiring the user's Mac/CapCut/footage may be marked `BLOCKED BY EVIDENCE`; they must never be marked passed by simulation alone.
...
- Detect chip, RAM, free storage, CapCut version/project root and Claude CLI status.
...
- Generate Faithful, Balanced and Bold candidates from the same fixture.
- Pass: all mandatory script facts remain; every `COACH_IDEA` has a reason/confidence and can be removed independently; Faithful contains no unapproved experimentation; rejected idea types reduce future proposal weight without corrupting Style DNA.
...
A first-time user must install, open, process, continue in CapCut, export and review without Terminal after bootstrap.
...
- A beginner adds a script ZIP, raw folder/selected Drive folder and finished reference, confirms automatic grouping and understands which files are temporary.
...
- Premium master-template snapshot without personal paths/media;
