...
For this one-user local application, mandatory additional cost should be **$0 per month** beyond subscriptions/storage the owner already chooses to maintain.

The owner already has:

- Apple-silicon M2 Mac;
- CapCut Desktop Premium;
...
- React/TypeScript;
- local macOS helper;
- selected compatible CapCut adapter.

Licensing and redistribution obligations still apply even when software is free.

## 6. Optional costs, never mandatory

- Additional Google Drive storage if the owner chooses to retain more cloud media.
- External SSD if local working space becomes inconvenient.
- Claude usage credits beyond the included allowances, only after the owner explicitly enables a monthly cap.
- Apple Developer membership only if the owner later wants wider signed/notarized distribution; personal local use does not put this on the v1 critical path.
